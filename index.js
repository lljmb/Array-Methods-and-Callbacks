...
function getWinnersByYear(year,winner){
 ...
 let ByYear = getFinals.reduce(function(str,year,winner) {
  return str +  `In ${year.Year}, ${getWinners.winner[i]} won the world cup \n`;
}, "") 
return ByYear;
}
...
const getAverageGoals = fifaData.reduce((total, goal) => {
    let sum = total + goal['Home Team Goals'] || total + goal['Away Team Goals'];
    return sum;
  }, 0)
  
  const average = getAverageGoals / fifaData.length;
  
  console.log("the average number of goals scored per game is: " +average)
